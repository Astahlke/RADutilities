...
# usage is flip_trim_sbfI_190416.pl barcodes.txt infile1.fastq infile2.fastq outfile1.fastq outfile2.fastq rejects1.fastq rejects2.fastq
...
open (OUT3, ">$ARGV[5]");   # create fastq outfile for rejected reads (from original R1)
open (OUT4, ">$ARGV[6]");   # create fastq outfile for rejected reads (from original R2)
...
while($name1 = <IN1>) {		# start reading through the paired fastq input files
	...
	if($which == 1) {						# if the pair is correctly oriented, print out fastq format for the pair
		...
		print OUT3 "$name1$forward$third1$qual1";
		print OUT4 "$name2$reverse$third2$qual2";
	}
}
close IN1;
close IN2;
